...
- [x] 장바구니 페이지 (Classic A 레이아웃: 테이블 + 우측 sticky Summary)
- [x] 체크아웃 페이지 (배송지 입력, 쿠폰/포인트 선택, 결제수단 — 단일열 + 우측 sticky)
- [x] 결제 진행 / 완료 / 실패 상태 페이지 (폴링 → 상태 분기)

---

## ✅ 03. Auth — MyPage
- [x] 로그인 / 회원가입 페이지
- [x] 마이페이지 홈 대시보드 (통계카드, 진행중 주문, 리뷰 대기)
- [x] 주문 목록 / 주문 상세 페이지 (배송 타임라인, 결제내역)
- [x] 리뷰 목록 / 리뷰 작성 화면
- [x] 위시리스트 페이지 (그리드 레이아웃)
- [x] 쿠폰함 / 포인트 이력 페이지
