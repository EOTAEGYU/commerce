...
### 2. 로그인 (signIn)
1. 이메일로 회원 조회 → 없으면 `INVALID_CREDENTIALS` 예외
2. 비밀번호 BCrypt 검증 → 불일치 시 `INVALID_CREDENTIALS` 예외
3. JWT 생성 (userId, email, role 포함)
4. `AuthResponse(accessToken)` 반환
...
| `INVALID_CREDENTIALS` | 401 | 이메일 없거나 비밀번호 불일치 |
