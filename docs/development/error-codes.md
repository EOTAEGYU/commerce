...
| `DUPLICATE_USERNAME` | 409 | 이미 사용 중인 아이디입니다. | 이미 사용 중인 username으로 회원가입 시도 |
| `DUPLICATE_PHONE` | 409 | 이미 사용 중인 전화번호입니다. | 이미 가입된 전화번호로 회원가입 시도 |
| `USER_NOT_FOUND` | 404 | 존재하지 않는 회원입니다. | userId로 회원 조회 실패 |
| `INVALID_CREDENTIALS` | 401 | 아이디 또는 비밀번호가 올바르지 않습니다. | 로그인 시 아이디 없거나 비밀번호 불일치 |
...
| 코드 | HTTP 상태 | 메시지 | 발생 상황 |
|------|----------|--------|----------|
| `POINT_INSUFFICIENT` | 400 | 포인트 잔액이 부족합니다 | 잔액보다 많은 포인트 사용 시도 |
| `POINT_BELOW_MINIMUM` | 400 | 최소 1,000 포인트 이상 사용해야 합니다 | 1,000 미만 포인트 사용 시도 |
